// app.js (Final Robust Version - Index Method)
...
// ==========================================
// Kita simpan data artikel di sini agar tombol tidak error
...
// ==========================================
// 3. FUNGSI LOGIKA (LOGIC FUNCTIONS)
// ==========================================

// --- RENDER CONTENT (FIX: MENGGUNAKAN INDEX BUKAN STRING) ---
async function renderContent() {
  // Simpan data di window agar bisa diakses global (Anti Error)
  window.globalContentData = [];
  ...
  const mkCard = (x, isArt, index) => {
    ...
    // Perbaikan: Kita kirim INDEX array, bukan string data
    return `
  ...
  if (pW) {
    ...
    // Tambahkan ke global data dengan penanda tipe
  ...
  if (aL) {
    ...
    // Filter Logic
    const filter = (q) => {
      // Kita perlu map ulang karena index global berubah jika difilter
      // Untuk sederhananya, kita render ulang dari globalData yang cocok
      // Tapi karena struktur search sederhana, kita pakai cara aman:
      // Render ulang HANYA artikel yang cocok, tapi ID tetap mengacu ke global
      
      const filtered = d.map((item, localIdx) => ({item, globalIdx: startIdx + localIdx}))
                        .filter(obj => (obj.item.title || "").toLowerCase().includes(q));
      
      aL.innerHTML = filtered.length ? filtered.map(obj => mkCard(obj.item, true, obj.globalIdx)).join("") : "";
...
// --- FUNGSI GLOBAL POPUP (DIPANGGIL DARI HTML) ---
window.openArticleModal = (index) => {
  try {
    const data = window.globalContentData[index]; // Ambil dari Array, bukan JSON Parse
...
function initZakatCalculator() {
  ...
  if (!openBtn || !modal) return; // Jika elemen tidak ada, stop (biar gak error)
  ...
  if(priceInput) priceInput.value = DEFAULT_JPY; // Default value
