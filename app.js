// app.js (Fixed Version - All Functions Included & Ordered)

const $ = (s, r = document) => r.querySelector(s);
const $$ = (s, r = document) => [...r.querySelectorAll(s)];

// ==========================================
// 1. KONFIGURASI & DATA (Ditaruh Paling Atas)
// ==========================================
...
const TRANSLATIONS = {
  id: {
    ...
    hadith_label: "Mutiara Hadits", sholat_title: "Jadwal Sholat", gallery_title: "Galeri Foto",
    gallery_desc: "Dokumentasi kegiatan dan kebersamaan jamaah.", tab_announcement: "Pengumuman",
    tab_article: "Artikel & Faedah", empty_data: "Belum ada data terbaru.", empty_search: "Tidak ditemukan.",
    donasi_badge: "Peluang Amal Jariyah", donasi_title: "Investasi Kekal Akhirat",
    deadline_label: "Batas Waktu Wakaf (Tahap 1)", progress_title: "Progres Pembangunan",
    collected: "Terkumpul", needed: "Kekurangan", confirm_title: "Konfirmasi Donasi",
    confirm_desc: "Masukkan nominal yang telah ditransfer.", or: "ATAU", btn_confirm: "Konfirmasi via WA",
    footer_links: "Tautan", footer_follow: "Lokasi", btn_zakat: "Hitung Zakat",
    btn_donate_now: "Donasi Sekarang", btn_popup_wakaf: "Ikut Wakaf Sekarang",
    ...
    freq_once: "Sekali Saja", freq_monthly: "Rutin Tiap Bulan", btn_save_reminder: "Pasang Pengingat",
    reminder_note: "*Akan membuka Google Calendar Anda.", reminder_title: "✨ Komitmen Wakaf Masjid Hekinan",
  ...
  en: {
    ...
    hadith_label: "Daily Hadith", sholat_title: "Prayer Times", gallery_title: "Photo Gallery",
    gallery_desc: "Documentation of community activities.", tab_announcement: "Announcements",
    tab_article: "Articles", empty_data: "No updates.", empty_search: "Not found.",
    donasi_badge: "Charity Opportunity", donasi_title: "Invest for Hereafter",
    deadline_label: "Donation Deadline", progress_title: "Construction Progress",
    collected: "Collected", needed: "Remaining", confirm_title: "Confirm Donation",
    confirm_desc: "Enter transferred amount.", or: "OR", btn_confirm: "Confirm via WA",
    footer_links: "Links", footer_follow: "Location", btn_zakat: "Zakat Calculator",
    btn_donate_now: "Donate Now", btn_popup_wakaf: "Donate Now",
    ...
    freq_once: "One Time", freq_monthly: "Monthly (Recurring)", btn_save_reminder: "Set Reminder",
    reminder_note: "*Opens Google Calendar.", reminder_title: "✨ Donation Commitment Hekinan Mosque",
...
// ==========================================
// 2. HELPER FUNCTIONS (DEFINISI AWAL)
// ==========================================
  ...
  const mkCard = (x, isArt) => {
    ...
    return `
      <article class="group rounded-2xl border border-slate-100 bg-white p-5 shadow-sm hover:border-sky-200 hover:shadow-md flex flex-col h-full transition-all duration-300">
        ...
        <button onclick="openArticleModal('${dataString}')" class="mt-auto flex items-center gap-1.5 text-sm font-bold text-fig-primary hover:text-fig-primaryDark hover:underline transition-all w-fit"><span data-i18n="read_more">Selengkapnya</span><i data-lucide="arrow-right" class="w-4 h-4 transition-transform group-hover:translate-x-1"></i></button>
      </article>`;
  };
  const pW = $("#wrapPengumuman");
  if (pW) { const d = await loadCsv(getCsvUrl("pengumuman")); pW.innerHTML = d.length ? d.map(x => mkCard(x, false)).join("") : ""; if (!d.length) $("#boardEmpty")?.classList.remove("hidden"); }
...
window.openArticleModal = (dataString) => {
  try {
    ...
    $("#modalTitle").textContent = data.title || ""; $("#modalDate").innerHTML = `<i data-lucide="calendar" class="w-3 h-3"></i> ${data.date || "Tanpa Tanggal"}`; $("#modalTag").textContent = data.tag || "Info";
    let contentHTML = data.content || data.desc || data.excerpt || ""; contentHTML = contentHTML.replace(/\n/g, "<br>"); $("#modalContent").innerHTML = contentHTML;
    const extBtn = $("#modalExternalLink");
    if (data.link && data.link.startsWith("http")) { extBtn.href = data.link; extBtn.classList.remove("hidden"); extBtn.classList.add("flex"); } else { extBtn.classList.add("hidden"); extBtn.classList.remove("flex"); }
...
document.addEventListener("DOMContentLoaded", () => {
  const modal = $("#articleModal"); const close = () => { if (modal) { modal.classList.add("hidden"); modal.classList.remove("flex"); } };
...
// --- DONASI LOGIC (WIDGET ORANG, DEDIKASI, REMINDER) ---
function initDonasi() {
...
function initHeroSlider() { const slides = $$(".hero-slide"); if (slides.length < 2) return; let current = 0; setInterval(() => { slides[current].classList.remove("active"); current = (current + 1) % slides.length; slides[current].classList.add("active"); }, 5000); }
function initCountdown() { const end = new Date("2026-05-31T23:59:59").getTime(); setInterval(() => { const gap = end - new Date().getTime(); if (gap < 0) return; $("#cdDays").innerText = Math.floor(gap / 86400000); $("#cdHours").innerText = Math.floor((gap % 86400000) / 3600000); $("#cdMin").innerText = Math.floor((gap % 3600000) / 60000); }, 1000); }
